...
A reverse-upstream mux has no ordinary client allocation-age limit and permits all 224 data-channel IDs as both total and active channels. The initiating server replaces its primary reverse mux after 24 hours or after more than half the IDs have been allocated. It authenticates and sends the route update on the replacement before retiring the older mux. The older mux then stops refresh-idle keepalives but may drain existing channels until normal idle handling closes it. Before retirement or the first channel request, refresh-idle keepalives preserve the reverse mux; after channel traffic starts, keepalive returns to normal mode. A non-empty channel request counts as real data activity.
